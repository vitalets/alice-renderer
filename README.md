...
**Параметры:**
  * **str** `{String|Array<String>}` - строка текста или массив строк, из которого будет выбран случайный элемент.

Удобно использовать совместно с `tts()`:
```js
...
**Параметры:**
  * **str** `{String|Array<String>}` - строка для голоса, или массив строк, из которого будет выбран случайный элемент.

Удобно использовать совместно с `text()`:
```js
reply`Вы можете написать нам на емейл${text(' user1234@example.com')}. ${tts('Он на вашем экране.')}`;
